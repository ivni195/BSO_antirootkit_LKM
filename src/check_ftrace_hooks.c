...
static const unsigned char nop_5byte[] = { 0x0f, 0x1f, 0x44, 0x00, 0x00 };

static bool is_mod_whitelisted(const char *name)
{
	int i;
	for (i = 0; i < NUM_WHITELISTED; i++) {
		if (strncmp(name, whitelisted_mods[i],
			    strlen(whitelisted_mods[i])) == 0)
			return true;
	}
...
void scan_for_ftr_calls(void)
{
	...
	for (i = 0; i < NUM_PROTECTED_FUNCS; i++) {
		...
		if (ops == NULL) {
			pr_debug(
				"Failed to lookup ftrace_ops for %s function hook.",
				name);
			continue;
		}
		...
		// If we can't find module, we treat the module as hidden and remove the hook anyway.
			...
			rk_info("Function %s is hooked but the hooking module (%s) is whitelisted.",
				name, mod->name);
			continue;
		}
		...
		rk_warning(
			"Function %s is hooked and the hooking module is not whitelisted! Attempting to remove the hook.",
			name);
		// Remove hook
		if (unregister_ftrace_function(ops) != 0) {
			rk_warning(
				"Failed to unregister ftrace hook for %s function.",
				name);
		} else {
