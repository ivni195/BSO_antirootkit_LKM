...
int compare_sys_call_table(void)
{
	...
	for (i = 0; i < __NR_syscall_max; ++i) {
		if (IS_ENTRY_HOOKED(i)) {
			...
			rk_warning(
				"Looks like %s has been hooked.\nThe address should be: 0x%pK\nbut instead it is: 0x%pK",
				buff, (void *)sys_call_table_saved[i],
				(void *)sys_call_table[i]);
			changed = true;
		} else if (!core_kernel_text_(sys_call_table[i])) {
			sprint_symbol(buff, sys_call_table_saved[i]);
			rk_warning(
				"Looks like %s is not originated in the core kernel text section.",
				buff);
			changed = true;
...
void restore_sys_call_table(int action)
{
	...
	if (action == 1) {
		...
		disable_memory_protection();
		rk_warning(
			"Syscall hooks found. Backup entries point to hooks. Trying to recover by searching memory...");
